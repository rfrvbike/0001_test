...
const technicalPanelSource = readFileSync("src/components/TechnicalPanel.js", "utf8");
const policyThemePanelSource = readFileSync("src/components/PolicyThemePanel.js", "utf8");
const stockAnalyzerHtml = readFileSync("stock-analyzer.html", "utf8");
const stockAnalyzerMainSource = readFileSync("src/main.js", "utf8");
assert.equal(stockAnalyzerHtml.includes('id="app"'), true);
assert.equal(stockAnalyzerHtml.includes('type="module"'), true);
...
assert.equal(stockAnalyzerSource.includes("#fetchJquantsMasterMockBtn"), true);
assert.equal(stockAnalyzerSource.includes("#dryRunJquantsMasterBtn"), true);
assert.equal(stockAnalyzerSource.includes("function safeLower"), true);
assert.equal(stockAnalyzerSource.includes("function safeClassToken"), true);
assert.equal(stockAnalyzerSource.includes("stock.dataSource.toLowerCase()"), false);
assert.equal(stockAnalyzerSource.includes("safeClassToken(stock.dataSource)"), true);
assert.equal(technicalPanelSource.includes("function formatFixedOrMissing"), true);
assert.equal(technicalPanelSource.includes("indicators.volumeRatio.toFixed"), false);
assert.equal(technicalPanelSource.includes("indicators.rsi.toFixed"), false);
assert.equal(policyThemePanelSource.includes("function safeClassToken"), true);
assert.equal(policyThemePanelSource.includes("stock.policyRelationType.toLowerCase()"), false);
