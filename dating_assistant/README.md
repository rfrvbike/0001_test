...
1. 相手プロフィールを文字情報として登録する
2. 情報が少ないプロフィールでも1件の相手として保存する
3. 登録した相手を選び、次に送る候補A/B/Cを作る
4. 実際に手動送信した文だけを送信済みとしてlocal記録する
5. 相手から返信が来たら会話履歴に追加する
6. 相手別メモや送信結果メモを残す
...
1. 「プロフィール登録」を開きます。
2. ChatGPTで整理したプロフィール文、またはアプリ上で読める文字情報を貼り付けます。
3. 保存前プレビューを確認し、保存します。
4. 保存後は自動で会話対象になり、「相手と会話する」画面へ進めます。
5. 候補A/B/Cを作り、人間が確認してからマッチングアプリ上で手動送信します。
6. 送信後だけ、このGUIで送信済みlocal記録を行います。

### データ保存と安全な使い方

入力したプロフィール、会話履歴、送信済み記録、メモは、基本的にお使いのPC内にlocal保存されます。実プロフィールや会話履歴のlocalデータはGit管理対象に入れない方針です。

本名、勤務先、学校名、LINE ID、SNS ID、住所、電話番号、メールアドレスなどは保存しないでください。画像から読み取る機能を使う場合も、画像そのもの、顔写真、スクリーンショットそのものは保存しません。読み取った文字をユーザーが確認し、必要な範囲だけ保存してください。

### よくある困りごと

- 保存できない場合: 貼り付け欄も補助入力欄も完全に空の場合だけ保存しません。自己紹介、写真メモ、趣味など、分かる範囲で1つ以上入力してください。
- 情報が少ない場合: warningは出ますが、情報が少ないプロフィールとして保存できます。あとから補完できます。
- 候補が出ない場合: 相手が選ばれているか、プロフィールや会話履歴がlocal保存されているかを確認してください。
- 自動送信が心配な場合: このGUIは送信しません。候補文を確認し、実際の送信はユーザー本人がマッチングアプリ上で手動で行います。

### 現時点の制限

実LLM API呼び出し、外部API連携、マッチングアプリへの自動接続、自動送信、画像そのものの保存、課金機能、ユーザー認証機能は現在の対象外です。候補文は最終的に利用者本人が確認して使ってください。相手の反応や返信は保証しません。

## No.150 購入者向けの画面構成

GUIのタブは、購入者が迷わず使える順番に整理しています。

1. 相手と会話する: 普段使うメイン画面です。相手を選び、プロフィールと会話履歴を見ながら、次に送る候補A/B/Cを作ります。
2. プロフィール登録: 初回や新しい相手を登録するときに使います。ChatGPTで整理したプロフィール文や、アプリ上で読める文字情報を貼り付けて保存します。
3. 登録済み相手の管理: 登録済みの相手を検索・整理します。表示名やメモの修正、非表示/再表示ができます。
4. 会話履歴追加: 相手から返信が来たときや、過去のやり取りをまとめて追加したいときに使います。
5. 設定・ヘルプ: このツールでできること、できないこと、安全な使い方、local保存の考え方を確認できます。

実際の送信は、ユーザー本人がマッチングアプリ上で手動で行います。GUIは候補生成とlocal記録だけを行い、自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作は行いません。

## No.147 候補文の使い分け

候補A/B/Cは、どれか1つを人間が確認して手動送信するための候補です。候補Aは一番無難で、迷ったときに使いやすい文です。候補Bは少し親しみやすく、相手の話題を広げたいときに向いています。候補Cは少し距離を縮める文ですが、初回や温度感が低い場面では慎重に確認してください。

情報が少ない相手には、プロフィールにない趣味や性格を決めつけず、「プロフィールの雰囲気が印象に残りました」程度の短い初回候補に留めます。趣味が分かる相手には、カフェ、映画、散歩など相手プロフィール内の話題に自然に触れ、質問は1つまでにします。

電話、会う提案、LINE交換は、会話が続いていて相手の温度感が普通以上のときだけ検討します。候補文には短時間、断りやすさ、「アプリのままでも大丈夫」などの余白を入れます。少し大人っぽい雰囲気は、恋愛感や余韻を控えめにするだけで、身体評価、下ネタ、ホテル・自宅を連想させる表現は使いません。

実際の送信はユーザー本人がマッチングアプリ上で手動で行います。GUIは候補生成とlocal記録だけを行い、自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作は行いません。

## No.146 本番前の実画面確認

販売前提の最終確認として、`start_dating_assistant_gui.bat` から起動し、プロフィール登録から「相手と会話する」画面、初回候補生成、送信済みlocal記録、会話履歴追加までを実画面に近い流れで確認します。

通常操作では、`label` や保存用の内部IDを入力する必要はありません。プロフィール保存の確認欄も、購入者が分かりやすいように「このプロフィールをlocal保存する」と表示します。保存後は自動で会話対象になり、「この相手と会話する」導線から次に進めます。

情報が少ないプロフィールでも保存できます。不足項目はwarningとして表示されますが、完全空欄でなければ1件のプロフィールデータとして扱い、あとから補完できます。候補生成や送信済み記録はlocal保存のみで、自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作は行いません。
...
「登録済み相手の管理」は、保存済みプロフィールや会話対象の確認・整理が必要な場合だけ使う補助画面です。通常運用の主導線は「プロフィール登録」から「相手と会話する」へ進みます。
...
GUI版では、プロフィール貼り付け、会話対象の登録、会話履歴管理、3候補生成、送信済みlocal記録、送信結果メモまでをブラウザ画面で操作できます。
...
1. まず「プロフィール登録」を開きます。
2. ChatGPTプロジェクトで整理したプロフィール情報、またはマッチングアプリ上で読める自己紹介・趣味・写真の印象メモを貼り付けます。
3. 情報が少なくても保存できます。保存IDやlabelは入力不要で、dating_assistantが自動生成します。
4. 保存すると、自動で会話対象として登録されます。
5. 次に「相手と会話する」で候補A/B/Cを作ります。
6. 実際の送信はマッチングアプリ上でユーザー本人が手動で行い、送った後だけGUIでlocal記録します。
7. 相手から返信が来たら、同じ「相手と会話する」画面内の会話履歴追加、または会話履歴インポートに貼り付けます。

登録済み相手の管理:

- 「プロフィール管理」では、登録済みの相手プロフィールと会話対象を確認・整理できます。
- 通常の候補作成は「相手と会話する」で行います。「プロフィール管理」は補助的な管理画面です。
- 登録済み相手一覧では、表示名、アプリ名、年齢、エリア、情報量、会話状態、最終更新、最後に送った日、メモの有無を確認できます。
- 表示名検索、アプリ名、状態、情報少なめだけ表示で絞り込めます。
- 表示名やアプリ名、管理メモはあとから更新できます。内部保存IDや会話履歴、送信済み記録は変更しません。
- 会話が終わった相手や間違えて登録した相手は、まず「非表示」にして整理します。非表示は完全削除ではなく、会話履歴や送信済み記録を残します。
- 非表示中の相手は「非表示中の相手も一覧に含める」で確認し、必要なら再表示できます。
- 完全削除は誤操作のリスクが高いため、通常運用では使いません。
...
4. 保存すると、自動で会話対象として登録されます。通常はプロフィール管理で手動作成する必要はありません。
5. 必要に応じて、会話履歴インポートで既存のやり取りを会話対象へ記録します。
6. 「相手と会話する」で相手を選択し、プロフィール、会話履歴、timeline、pending_suggestionsを確認します。
