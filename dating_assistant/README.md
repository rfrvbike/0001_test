...
1. GUIを起動します。
2. プロフィール登録タブで、相手プロフィールのテキストや写真印象メモをまとめて貼り付けます。
3. 抽出プレビューを確認し、不足分だけ手入力してreal profileとして保存します。
...
7. 目的と文章の雰囲気を選び、初回候補または返信候補を3つ生成します。
8. 候補文を人間が確認し、必要なら短く自然な文へ整えます。
9. 実際のマッチングアプリ上では、ユーザー本人が手動で送信します。
10. 手動送信した後だけ、GUIで送信済みlocal記録を行います。
11. 相手から返信が来たら、会話履歴インポートまたは相手返信追加で記録し、次の返信候補を生成します。
12. 使わなかった候補は、必要に応じて候補破棄で整理します。
...
- 電話、会う提案、LINE交換は目的として選べますが、GUIは早すぎる可能性を表示します。実際に送るかはユーザーが判断してください。
