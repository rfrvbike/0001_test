...
- README先頭に、購入者が最初に読む「できること」「できないこと」「起動方法」「初めて使うときの流れ」「データ保存と安全な使い方」「よくある困りごと」「現時点の制限」を追加した。
- 設定・ヘルプタブに、起動できないとき、自動送信されるか、入力データの保存先、画像保存の有無、情報が少ないプロフィールの扱いをFAQとして追加した。
- 通常画面では、自動送信ではなく手動送信後のlocal記録であること、外部API通信や実LLM API呼び出しを行わないこと、画像そのものを保存しないことを確認できる構成にした。
- data/local/ や outputs/local/、実プロフィールYAML、partner実データYAMLはcommit対象にしていない。
- 自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作機能は追加していない。

## 作業No.150 タブ構成・画面名・ヘルプ導線の最終整理

- 購入者・一般ユーザー目線で、タブ構成、画面名、各画面上部の説明、初回利用者と継続利用者の導線を確認した。
- タブ構成を「相手と会話する」「プロフィール登録」「登録済み相手の管理」「会話履歴追加」「設定・ヘルプ」に整理した。
- 各画面上部に、この画面でできることと、実際の送信は手動であることが分かる短い説明を維持・追加した。
- GUI内に「設定・ヘルプ」タブを追加し、できること、できないこと、基本の使い方、安全な使い方、local保存の考え方を確認できるようにした。
- 会話履歴追加画面では、通常表示の `partner` / `conversation_history` など開発者向け表現を減らし、相手・会話履歴・local記録という購入者向け表現へ寄せた。
- README上部に購入者向けの画面構成と最短操作説明を追記した。
- 自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作、画像保存、顔写真保存、スクリーンショット保存は追加していない。

## 作業No.149 登録済み相手管理UIの販売前改善

- 購入者・一般ユーザー目線で、「保存済みプロフィール管理」を登録済み相手の確認・整理用画面として見直した。
- 管理画面の役割を、通常の候補作成画面ではなく、登録済みプロフィールと会話対象を確認・整理する補助画面として明記した。
- 登録済み相手一覧に、表示名、アプリ名、年齢、エリア、情報量、会話状態、ステータス、最終更新、最後に送った日、メモ有無を表示するようにした。
- 表示名・趣味・エリア検索、アプリ名絞り込み、状態絞り込み、情報少なめだけ表示、非表示中の相手を含める表示を追加した。
- 管理画面から「この相手と会話する」導線、表示名・アプリ名・管理メモ更新、非表示/再表示の操作を追加した。
- 完全削除は実装せず、誤操作防止のため会話履歴や送信済み記録を残すアーカイブ/非表示を優先した。
- 自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作、画像保存、顔写真保存、スクリーンショット保存は追加していない。

## 作業No.148 初回利用導線の販売前確認

- 購入者・一般ユーザー目線で、初回起動時、空データ時の「相手と会話する」画面、プロフィール登録、保存後導線、プロフィール管理、会話履歴追加、自動送信ではない説明を確認した。
- 初回画面に「まずプロフィール登録から始める」「保存後に相手と会話するで候補を作る」「実際の送信はユーザー本人が手動で行う」という短い案内を追加した。
- 空データ時の「相手と会話する」画面では、エラーや内部IDではなく、プロフィール登録から相手を追加する次アクションを表示するように文言を調整した。
- プロフィール登録画面では、ChatGPTプロジェクトで整理したプロフィール情報を貼る場所、情報が少なくても保存できること、保存IDやlabel入力が不要であることを明示した。
- 保存後の案内では、候補はlocalに記録されるだけで自動送信ではないこと、実際の送信はマッチングアプリ上で手動で行うことを再明記した。
- READMEに、初めて使う人向けの最短操作手順と、プロフィール保存後は自動で会話対象になる現在の基本フローを反映した。
- 自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作、画像保存、顔写真保存、スクリーンショット保存は追加していない。

## 作業No.147 候補文品質の実運用確認

- 購入者・一般ユーザー目線で、情報少なめプロフィール、趣味ありプロフィール、会話履歴あり返信、電話提案、会う提案、LINE交換、少し大人っぽい雰囲気の候補文品質を確認した。
- 情報が少ない相手には、プロフィールにない情報を盛らず、短めで負担の少ない初回候補に留めることを確認した。
- 趣味ありプロフィールでは、カフェや映画など相手プロフィール内の話題に自然に触れ、質問を1つに絞る方針を確認した。
- 会話履歴あり返信では、相手の「静かなカフェ」など直近返信の要素を拾い、こちらの話を少しだけ混ぜる候補に調整した。
- 電話、会う提案、LINE交換、少し大人っぽい雰囲気が許可される段階でも、候補A/B/Cが同じ文にならないようにし、断りやすさ、短時間、アプリのままでもよい余白を入れた。
- 大人っぽい候補は露骨な性的表現、身体評価、ホテル・自宅連想を避け、控えめな恋愛感に留める方針を確認した。
- 自動送信、外部API通信、実LLM API呼び出し、マッチングアプリ操作、画像保存、顔写真保存、スクリーンショット保存は追加していない。
