...
販売前ベータ配布前チェックでは、実データ、local保存先、secret、cache、一時ファイル、画像そのものがGit管理対象や購入者向け説明に混ざらないことを確認する。trackedに残してよいのは、example、.gitkeep、ドキュメント、テスト、コードなど実データを含まないものに限定する。

購入者向けREADME上部と設定・ヘルプでは、触るものを `start_dating_assistant_gui.bat`、ブラウザGUI、README上部、必要に応じたプロフィール文に絞って案内する。通常利用では、src、tests、reports、data/local、outputs/local、YAML、Python、.env、Git関連ファイルを開いたり編集したりする必要がないことを明記する。

## 作業No.151 方針追記

販売前ベータ版では、README上部と「設定・ヘルプ」タブを購入者が最初に読む説明として扱う。できること、できないこと、起動方法、初回の流れ、普段の使い方、local保存、安全上の注意、困ったときの対処を短く分かりやすく置く。

通常画面とREADME上部では、partner YAML、raw JSON、payload、debug、import preflight、AppTest、internal pathなどの開発者向け表現を前面に出さない。必要な詳細はREADME下部、開発者向け補足、または折りたたみ表示へ分離する。

販売前ベータ版の制限として、自動送信、マッチングアプリへの直接接続、外部API通信、実LLM API呼び出し、画像そのものの保存、顔写真保存、スクリーンショット保存、課金、ユーザー認証、販売ページ作成は行わないことを明記する。

## 作業No.150 方針追記

販売前提の最終画面構成では、普段使う「相手と会話する」を先頭に置き、初回登録用の「プロフィール登録」、整理用の「登録済み相手の管理」、補助用の「会話履歴追加」、不安解消用の「設定・ヘルプ」を並べる。初回利用者にも継続利用者にも、次に押す場所が分かる画面名を優先する。

通常画面では、partner_id、label、YAML、raw JSON、internal status、debugなどを主役にしない。必要な詳細は折りたたみに置き、購入者には「相手」「会話履歴」「候補」「送信済みlocal記録」「非表示」などの行動名で見せる。実際の送信は手動で、自動送信、外部通信、実LLM API呼び出し、マッチングアプリ操作は追加しない。

## 作業No.149 方針追記

販売前提の相手管理UIでは、複数人を登録しても「誰が登録されているか」「次に何をする相手か」「情報が少ない相手か」が一覧で分かることを優先する。通常画面ではpartner_id、label、YAML、raw JSONを主役にせず、表示名、アプリ名、情報量、会話状態、最終更新、最後に送った日、メモ有無を購入者向けラベルで表示する。

表示名やアプリ名、管理メモは後から更新できるようにするが、内部保存ID、会話履歴、送信済み記録は変更しない。会話が終わった相手の整理は完全削除ではなく、まずアーカイブ/非表示を優先し、確認チェックを入れて誤操作を防ぐ。通常の候補作成は「相手と会話する」画面で行う方針を維持する。
