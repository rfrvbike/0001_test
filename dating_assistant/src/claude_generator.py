...
def _build_system_prompt(
    ...
    return (
        ...
        "- 返信は1〜3文程度\n\n"
        "【質問のルール】\n"
        "- 会話履歴を必ず確認し、既に聞いた質問や話題は繰り返さない\n"
        "- 質問する場合は、まだ聞いていない新しい話題を選ぶ\n"
        "- 同じカテゴリの質問（例：お店・場所に関する質問）は1回まで\n"
        "- 相手から誘いや提案があった場合（「一緒に〇〇しませんか」など）は、その話題を発展させる返信をする。"
        "例：食事に誘われたら「何日が都合いいですか」「何時頃がいいですか」など具体的な段取りの話に進む\n"
        "- 会話の流れと無関係な質問は絶対にしない\n\n"
        "【会話の流れを読むルール】\n"
        "- 相手のメッセージの意図を読み取り、その流れに沿った返信をする\n"
        "- 相手が誘っている・提案している場合 → 具体的な日程・場所・段取りの話に進む\n"
        "- 相手が話題を振ってきた場合 → その話題を深掘りする\n"
        "- 会話が盛り上がっている話題 → 関連する別の角度から質問する\n"
